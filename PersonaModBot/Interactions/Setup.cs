...
namespace PersonaModBot.Interactions
{
    public class Setup : InteractionModuleBase
    {
        ...
        public Setup(EdgeDBClient db, InteractionHelper helper)
        {
            _db = db;
            _helper = helper;
        }
        ...
        [ComponentInteraction("setup-forums")]
        public async Task SelectForums(string[] selectedForums)
        {
            ...
            _helper.SetupForums.Add(interaction.Message.Id, selectedForums.Select(f => (ulong.Parse(f), (ulong)0)).ToArray());
        ...
        [ComponentInteraction("setup-solved-tag")]
        public async Task SetSolvedTag(string tagStr)
        {
            ...
            var forums = _helper.SetupForums[interaction.Message.Id];
        ...
        [ComponentInteraction("setup-roles")]
        public async Task SetupRoles(string[] selectedRoles)
        {
            IComponentInteraction interaction = (IComponentInteraction)Context.Interaction;
            _helper.SetupRoles.Add(interaction.Message.Id, selectedRoles.Select(role => new RoleConfig(ulong.Parse(role), false, false, false)).ToArray());

            var roles = Context.Guild.Roles.Where(role => selectedRoles.Contains(role.Id.ToString()));

            var selectedForums = _helper.SetupForums[interaction.Message.Id];
        ...
        [ComponentInteraction("setup-perms")]
        public async Task SetupPerms(string[] permissions)
        {
            IComponentInteraction interaction = (IComponentInteraction)Context.Interaction;

            var selectedRoles = _helper.SetupRoles[interaction.Message.Id];
            ...
            var buttons = new ComponentBuilder()
                .WithButton("I'm Done", "setup-done")
                .WithButton("Setup More Roles", "setup-solved-tag");
        ...
        [ComponentInteraction("setup-done")]
        public async Task SetupDone()
        {
            ...
            foreach (var role in _helper.SetupRoles[interaction.Message.Id])
            ...
            foreach (var forum in selectedForums)
            {
                await _db.QueryAsync<ForumConfig>(
                        ...
                        "else(\n" +
                        "   update ForumConfig set { solvedTag := <int64>$solvedTag, solvedMessage := <str>$solvedMessage, allowedRoles := roles }\n" +
                        ")",
